...
void Solver::DensStep()
{
	...
	//SWAP(dens_prev, dens)				//Swapping matrixes, because we want save the next result in dens, not in dens_prev.
	if (!TWalls2&&Tadvec2) SWAP(dens_prev, dens);
...
void Solver::VelStep()
{
	...
	if (!TWalls2&&Tadvec2) SWAP(u_prev, u);
	if (!TWalls2&&Tadvec2) SWAP(v_prev, v);
	if (!TWalls2&&Tadvec2) Advect(1, u, u_prev, u_prev, v_prev);
	if (!TWalls2&&Tadvec2) Advect(2, v, v_prev, u_prev, v_prev);
	
	if (TWalls2&&Tadvec2) Advect(1, u, u_prev, u, v);
	if (TWalls2&&Tadvec2) Advect(2, v, v_prev, u, v);
...
void Solver::LinSolve(int b, float * x, float * x0, float aij, float aii)
{
	...
	int j;
	for (i = 0; i <= N + 1; i++) {
		for (j = 0; j <= N + 1; j++)//Esto no es necesario, pero ayuda a una convergencia mejor
		{
	...
	for (int r = 0; r < 20; r++) {
		FOR_EACH_CELL
			x[XY_TO_ARRAY2(j, i)] = (1 / aii)*(aij*(x[XY_TO_ARRAY2(j - 1, i)] + x[XY_TO_ARRAY2(j, i - 1)]) 
											 + aij*(x[XY_TO_ARRAY2(j, i + 1)] + x[XY_TO_ARRAY2(j + 1, i)]) + x0[XY_TO_ARRAY2(j, i)]);
		END_FOR	
	}
}
...
//TODO: Se aplica el campo vectorial realizando una interploación lineal entre las 4 casillas más cercanas donde caiga el nuevo valor.
	...
	if (!TWalls2) {//caso no cícico

			for (j = 1; j < N + 1; j++) {
				for (i = 1; i < N + 1; i++)
				{
					x = (float)i - u[XY_TO_ARRAY2(j, i)] * dt*N;
					y = (float)j - v[XY_TO_ARRAY2(j, i)] * dt*N;
					if (x < 0.5f) x = 0.5f;
					if (y < 0.5f) y = 0.5f;
					...
					f3 = (1-fabs(a))*fabs(c);
					f2 = fabs(a)*(1 -fabs(c));
					f1 = (1-fabs(a))*(1-fabs(c));
		
					d[XY_TO_ARRAY2(j, i)] = f1*d0[XY_TO_ARRAY2(q, p)] + f2*d0[XY_TO_ARRAY2(q, p + 1)] + f3*d0[XY_TO_ARRAY2(q + 1, p)] + f4*d0[XY_TO_ARRAY2(q + 1, p + 1)];
		...
		for (j = 1; j < N + 1; j++) {
			for (i = 1; i < N + 1; i++)
			{
				...
				x = (float)i - u[XY_TO_ARRAY2(j, i)] * dt*N*factor1;
				y = (float)j - v[XY_TO_ARRAY2(j, i)] * dt*N*factor1;
				p = round(x);
				q = round(y);
				a = x-p;
				c = y-q;
				...
				if (!(p<1 || q<1 || p>N || q>N)) {
					...
					s1 = ((a > 0) - (a < 0));
					s2 = ((c > 0) - (c < 0));
					f4 = factor2*fabs(a)*fabs(c);
					f3 = factor2*(1-fabs(a))*fabs(c);
					f2 = factor2*fabs(a)*(1 -fabs(c));
					f1 = factor2*(1-fabs(a))*(1-fabs(c));
					s1 = p + s1;
					s2 = q + s2;
					s1 = MODY(s1);
					s2 = MODY(s2);
					...
					d[XY_TO_ARRAY2(j, i)] += sum0;
					d[XY_TO_ARRAY2(q, p)] -= sum0;

					sum0 = f2*d[XY_TO_ARRAY2(q, s1)];
					d[XY_TO_ARRAY2(j, i)] += sum0;
					d[XY_TO_ARRAY2(q, s1)] -= sum0;

					sum0 = f3*d[XY_TO_ARRAY2(s2, p)];
					d[XY_TO_ARRAY2(j, i)] += sum0;
					d[XY_TO_ARRAY2(s2, p)] -= sum0;

					sum0 = f4*d[XY_TO_ARRAY2(s2, s1)];
					d[XY_TO_ARRAY2(j, i)] += sum0;
					d[XY_TO_ARRAY2(s2, s1)] -= sum0;
